...
var TodoWhere = struct {
	...
	CreatedBy:  whereHelperint{field: "`todos`.`created_by`"},
	CreatedAt:  whereHelpertime_Time{field: "`todos`.`created_at`"},
	ModifiedBy: whereHelpernull_Int{field: "`todos`.`modified_by`"},
	ModifiedAt: whereHelpernull_Time{field: "`todos`.`modified_at`"},
...
var (
	todoAllColumns            = []string{"todo_id", "created_by", "created_at", "modified_by", "modified_at", "user_id"}
	todoColumnsWithoutDefault = []string{"created_by", "created_at", "modified_by", "modified_at", "user_id"}
	todoColumnsWithDefault    = []string{"todo_id"}
...
// TodoDetails retrieves all the todo_detail's TodoDetails with an executor.
func (o *Todo) TodoDetails(mods ...qm.QueryMod) todoDetailQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`todo_details`.`todo_id`=?", o.TodoID),
	)

	query := TodoDetails(queryMods...)
	queries.SetFrom(query.Query, "`todo_details`")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`todo_details`.*"})
	}
...
// LoadTodoDetails allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (todoL) LoadTodoDetails(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTodo interface{}, mods queries.Applicator) error {
	var slice []*Todo
	var object *Todo
	...
	query := NewQuery(qm.From(`todo_details`), qm.WhereIn(`todo_details.todo_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load todo_details")
	}

	var resultSlice []*TodoDetail
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice todo_details")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on todo_details")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for todo_details")
	}
	...
	if singular {
		object.R.TodoDetails = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &todoDetailR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.TodoID, foreign.TodoID) {
				local.R.TodoDetails = append(local.R.TodoDetails, foreign)
				if foreign.R == nil {
					foreign.R = &todoDetailR{}
				}
...
// AddTodoDetails adds the given related objects to the existing relationships
// of the todo, optionally inserting them as new records.
// Appends related to o.R.TodoDetails.
// Sets related.R.Todo appropriately.
func (o *Todo) AddTodoDetails(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*TodoDetail) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.TodoID, o.TodoID)
			...
			values := []interface{}{o.TodoID, rel.TodoDetailID}
	...
	if o.R == nil {
		...
		o.R.TodoDetails = append(o.R.TodoDetails, related...)
	}
...
func FindTodo(ctx context.Context, exec boil.ContextExecutor, todoID int, selectCols ...string) (*Todo, error) {
	...
	query := fmt.Sprintf(
		"select %s from `todos` where `todo_id`=?", sel,
	)
...
func (o *Todo) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	o.TodoID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == todoMapping["todo_id"] {
		goto CacheNoHooks
	}
...
func TodoExists(ctx context.Context, exec boil.ContextExecutor, todoID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `todos` where `todo_id`=? limit 1)"
