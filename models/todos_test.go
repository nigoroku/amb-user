...
func testTodosExists(t *testing.T) {
	...
	e, err := TodoExists(ctx, tx, o.TodoID)
	if err != nil {
...
func testTodosFind(t *testing.T) {
	...
	todoFound, err := FindTodo(ctx, tx, o.TodoID)
	if err != nil {
		t.Error(err)
	}
...
func testTodoToManyTodoDetails(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, todoDetailDBTypes, false, todoDetailColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, todoDetailDBTypes, false, todoDetailColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}

	queries.Assign(&b.TodoID, a.TodoID)
	queries.Assign(&c.TodoID, a.TodoID)
	...
	check, err := a.TodoDetails().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if queries.Equal(v.TodoID, b.TodoID) {
			bFound = true
		}
		if queries.Equal(v.TodoID, c.TodoID) {
			cFound = true
		}
	}
	...
	slice := TodoSlice{&a}
	if err = a.L.LoadTodoDetails(ctx, tx, false, (*[]*Todo)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TodoDetails); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.TodoDetails = nil
	if err = a.L.LoadTodoDetails(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TodoDetails); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTodoToManyAddOpTodoDetails(t *testing.T) {
	var err error
	...
	foreigners := []*TodoDetail{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, todoDetailDBTypes, false, strmangle.SetComplement(todoDetailPrimaryKeyColumns, todoDetailColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*TodoDetail{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddTodoDetails(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.TodoID, first.TodoID) {
			t.Error("foreign key was wrong value", a.TodoID, first.TodoID)
		}
		if !queries.Equal(a.TodoID, second.TodoID) {
			t.Error("foreign key was wrong value", a.TodoID, second.TodoID)
		}
		...
		if a.R.TodoDetails[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.TodoDetails[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.TodoDetails().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testTodoToManySetOpTodoDetails(t *testing.T) {
	var err error
	...
	err = a.SetTodoDetails(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetTodoDetails(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.TodoID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.TodoID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.TodoID, d.TodoID) {
		t.Error("foreign key was wrong value", a.TodoID, d.TodoID)
	}
	if !queries.Equal(a.TodoID, e.TodoID) {
		t.Error("foreign key was wrong value", a.TodoID, e.TodoID)
	}
	...
	if a.R.TodoDetails[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.TodoDetails[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testTodoToManyRemoveOpTodoDetails(t *testing.T) {
	var err error
	...
	err = a.AddTodoDetails(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveTodoDetails(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.TodoDetails) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.TodoDetails[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.TodoDetails[0] != &e {
