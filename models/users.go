...
var UserWhere = struct {
	...
	Introduction: whereHelperstring{field: "`users`.`introduction`"},
	Email:        whereHelperstring{field: "`users`.`email`"},
	Password:     whereHelperstring{field: "`users`.`password`"},
	Role:         whereHelperint8{field: "`users`.`role`"},
...
var (
	userAllColumns            = []string{"user_id", "account_name", "introduction", "email", "password", "role", "last_login_at", "created_by", "created_at", "modified_by", "modified_at", "account_img"}
	userColumnsWithoutDefault = []string{"account_name", "introduction", "email", "password", "role", "last_login_at", "created_by", "created_at", "modified_by", "modified_at", "account_img"}
...
func (o *User) InputAchievements(mods ...qm.QueryMod) inputAchievementQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`input_achievements`.`user_id`=?", o.UserID),
	)
...
// OutputAchievements retrieves all the output_achievement's OutputAchievements with an executor.
func (o *User) OutputAchievements(mods ...qm.QueryMod) outputAchievementQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`output_achievements`.`user_id`=?", o.UserID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`output_achievements`.*"})
	}
...
func (userL) LoadInputAchievements(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`input_achievements`), qm.WhereIn(`input_achievements.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load input_achievements")
	}

	var resultSlice []*InputAchievement
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice input_achievements")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on input_achievements")
	}
	...
	if singular {
		object.R.InputAchievements = resultSlice
...
// LoadOutputAchievements allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadOutputAchievements(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`output_achievements`), qm.WhereIn(`output_achievements.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load output_achievements")
	}

	var resultSlice []*OutputAchievement
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice output_achievements")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on output_achievements")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		object.R.OutputAchievements = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.UserID == foreign.UserID {
				local.R.OutputAchievements = append(local.R.OutputAchievements, foreign)
				if foreign.R == nil {
					foreign.R = &outputAchievementR{}
				}
				foreign.R.User = local
...
func (o *User) AddInputAchievements(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*InputAchievement) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.UserID, rel.InputAchievementID}
	...
	if o.R == nil {
		o.R = &userR{
			InputAchievements: related,
		}
	} else {
		o.R.InputAchievements = append(o.R.InputAchievements, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &inputAchievementR{
				User: o,
			}
		} else {
			rel.R.User = o
...
// Sets related.R.User appropriately.
func (o *User) AddOutputAchievements(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*OutputAchievement) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.UserID
			...
			values := []interface{}{o.UserID, rel.OutputAchievementID}
	...
	if o.R == nil {
		o.R = &userR{
			OutputAchievements: related,
		}
	} else {
		o.R.OutputAchievements = append(o.R.OutputAchievements, related...)
	}
