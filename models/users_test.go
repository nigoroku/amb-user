...
func testUserToManyInputAchievements(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, inputAchievementDBTypes, false, inputAchievementColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, inputAchievementDBTypes, false, inputAchievementColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.InputAchievements().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadInputAchievements(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.InputAchievements); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.InputAchievements = nil
	if err = a.L.LoadInputAchievements(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.InputAchievements); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyOutputAchievements(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, outputAchievementDBTypes, false, outputAchievementColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, outputAchievementDBTypes, false, outputAchievementColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.OutputAchievements().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadOutputAchievements(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OutputAchievements); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.OutputAchievements = nil
	if err = a.L.LoadOutputAchievements(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OutputAchievements); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyShareTokens(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, shareTokenDBTypes, false, shareTokenColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, shareTokenDBTypes, false, shareTokenColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.ShareTokens().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadShareTokens(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ShareTokens); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ShareTokens = nil
	if err = a.L.LoadShareTokens(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ShareTokens); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpInputAchievements(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e InputAchievement

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*InputAchievement{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, inputAchievementDBTypes, false, strmangle.SetComplement(inputAchievementPrimaryKeyColumns, inputAchievementColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*InputAchievement{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddInputAchievements(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.InputAchievements[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.InputAchievements[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.InputAchievements().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpOutputAchievements(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e OutputAchievement

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*OutputAchievement{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, outputAchievementDBTypes, false, strmangle.SetComplement(outputAchievementPrimaryKeyColumns, outputAchievementColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*OutputAchievement{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddOutputAchievements(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.OutputAchievements[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.OutputAchievements[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.OutputAchievements().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpShareTokens(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e ShareToken

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*ShareToken{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, shareTokenDBTypes, false, strmangle.SetComplement(shareTokenPrimaryKeyColumns, shareTokenColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*ShareToken{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddShareTokens(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ShareTokens[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ShareTokens[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ShareTokens().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`UserID`: `int`, `AccountName`: `varchar`, `Email`: `varchar`, `Password`: `varchar`, `Role`: `tinyint`, `LastLoginAt`: `timestamp`, `CreatedBy`: `int`, `CreatedAt`: `timestamp`, `ModifiedBy`: `int`, `ModifiedAt`: `timestamp`, `AccountImg`: `blob`, `Introduction`: `text`, `ContentType`: `varchar`}
	_           = bytes.MinRead
)
